...
instance Arbitrary Value where
  arbitrary = G.valueTypeGen >>= G.valueGen
...
testRowDecode = monadicIO $ 
                do update <- pick (G.doubleRFrameUpdateGen) -- we only want RFrameUpdate with Double as data
...
testOneHot = monadicIO $ 
                ...
                -- then call A.update and compare with the original
                do original <- pick (arbitrary :: Gen (RFrameUpdate Text Value)) 
                   -- we generate some data but really we will only care about its first column
                   ...
                   let 
                       -- we take the first column of the data, and choose random true and false values 
                       trueValue = (V.head <$> originalData) V.! indexTrue
                       falseValue = (V.head <$> originalData) V.! indexFalse

                       transformedData = (T.pack . show . V.head) <$> originalData -- we transform all data in the first column to Text
...
propTests = Ts.testGroup "Property tests"
