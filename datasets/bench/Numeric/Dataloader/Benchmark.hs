...
mkDataloaderWithIx :: Dataset h a -> IO (Dataloader 1 h a)
mkDataloaderWithIx ds = MWC.withSystemRandom $ \g -> do
  ixl <- uniformIxline ds g
  pure $ Dataloader Proxy (Just ixl) ds
  ...
  defaultMain
    [ bgroup "Numeric.Dataloader"
      ...
      , bgroup "testStream"
        ...
        , bench "no ixline"   . nfIO $ foldStream (Dataloader Proxy Nothing ds)
        ]
      ]
    ]
...
foldStream :: Show a => Dataloader 1 h a -> IO (Of [a] ())
